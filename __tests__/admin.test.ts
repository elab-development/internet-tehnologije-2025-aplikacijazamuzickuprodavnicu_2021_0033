...
vi.mock('@/app/actions/admin', () => ({
  ...
  getStatistikaProdajeProizvoda: vi.fn().mockResolvedValue({
    success: true,
    data: [
      { naziv: 'The Dark Side of the Moon', izvodjac: 'Pink Floyd', prihod: 7500, prodato: 3 },
      { naziv: 'Thriller', izvodjac: 'Michael Jackson', prihod: 5400, prodato: 3 }
    ]
  }),
}));
...
describe('API Admin - Izveštaji (GET)', () => {
  ...
  it('Trebalo bi vratiti 401 ako nema tokena', async () => {
    ...
    (cookies as ReturnType<typeof vi.fn>).mockReturnValue(Promise.resolve({ get: vi.fn().mockReturnValue(undefined) }));
...
describe('API Admin - Korisnici (GET)', () => {
  ...
  it('Trebalo bi vratiti 403 ako korisnik nije ADMIN', async () => {
    const token = createValidToken('PRODAVAC');
    const mockHeaders = new Map();
    mockHeaders.set('authorization', `Bearer ${token}`);
  ...
  it('Trebalo bi vratiti 200 sa listom korisnika ako je ADMIN', async () => {
    const token = createValidToken('ADMIN');
    const mockHeaders = new Map();
    mockHeaders.set('authorization', `Bearer ${token}`);
...
describe('API Admin - Dodaj Korisnika (POST)', () => {
  ...
  it('Trebalo bi vratiti 401 ako nema tokena', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/admin/korisnik', {
      method: 'POST',
      body: JSON.stringify({ ime: 'Marko', prezime: 'Marković', email: 'marko@test.com', lozinka: 'Test123!', uloga: 'KLIJENT' }),
  ...
  it('Trebalo bi vratiti 403 ako korisnik nije ADMIN', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/admin/korisnik', {
      method: 'POST',
      body: JSON.stringify({ ime: 'Petar', prezime: 'Petrović', email: 'petar@test.com', lozinka: 'Test123!', uloga: 'KLIJENT' }),
      headers: { 'x-csrf-token': 'valid-csrf-token' }
    });
  ...
  it('Trebalo bi vratiti 200 ako je ADMIN ulogovan', async () => {
    const token = createValidToken('ADMIN');
    const mockHeaders = new Map();
    mockHeaders.set('authorization', `Bearer ${token}`);
    ...
    const req = new NextRequest('http://localhost:3000/api/admin/korisnik', {
      method: 'POST',
      body: JSON.stringify({ ime: 'Jelena', prezime: 'Jelenić', email: 'jelena@test.com', lozinka: 'Test123!', uloga: 'PRODAVAC' }),
      headers: { 'x-csrf-token': 'valid-csrf-token' }
    });
