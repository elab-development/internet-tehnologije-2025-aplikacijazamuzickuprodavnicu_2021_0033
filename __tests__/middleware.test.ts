...
describe('Middleware - Integracioni Testovi', () => {
  ...
  describe('Javne Rute', () => {
    ...
    it('Dozvoli GET /api/proizvodi svima', async () => {
      const req = createRequest('/api/proizvodi', 'GET');
      const response = await middleware(req);
      expect(response?.status).toBe(200);
    });

    it('Zabrani POST /api/proizvodi bez tokena (401)', async () => {
      const req = createRequest('/api/proizvodi', 'POST');
      const response = await middleware(req);
  ...
  describe('Autorizacija po ulogama', () => {
    it('ADMIN ne može na /api/prodavac (403)', async () => {
      const token = await createToken('ADMIN');
      const req = createRequest('/api/prodavac/klijenti', 'GET', { token });
      const response = await middleware(req);
      expect(response?.status).toBe(403);
    });

    it('PRODAVAC ne može na /api/admin (403)', async () => {
      const token = await createToken('PRODAVAC');
      const req = createRequest('/api/admin/korisnici', 'GET', { token });
      const response = await middleware(req);
      expect(response?.status).toBe(403);
    });
    ...
    it('KLIJENT može na /api/klijent rute (200)', async () => {
      ...
      const req = createRequest('/api/klijent/kupljeni-proizvodi', 'GET', { token });
      const response = await middleware(req);
      expect(response?.status).toBe(200);
    });
  });
  ...
  describe('UI Zaštita (Redirects)', () => {
    ...
    it('Redirect logovanog korisnika sa /login na /', async () => {
      const token = await createToken('KLIJENT');
      const req = createRequest('/login', 'GET', { token, tokenInCookie: true });
      const response = await middleware(req);
      expect(response?.status).toBe(307);
      expect(response?.headers.get('location')).toContain('/stranice/svi-proizvodi');
    });
  });
